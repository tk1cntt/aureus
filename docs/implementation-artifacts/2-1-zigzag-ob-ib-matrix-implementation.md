...
- [ ] Task 1: Khởi tạo các biến trạng thái mới (AC: 2, 4)
  - Files
    - Modify: `services/aureus-signal/engine/common/zigzag_pro2.py`
  - [ ] Step 1: Thêm `self.ob_waiting`, `self.ob_high`, `self.ob_low`, `self.ob_index`, `self.ob_count` vào `__init__`.
  - [ ] Step 2: Thêm `self.last_piv_type` để theo dõi trạng thái Pivot gần nhất (Peak/Trough).

- [ ] Task 2: Triển khai logic Tiền xử lý OB (Pre-processing) (AC: 1, 2, 3, 5)
  - Files
    - Modify: `services/aureus-signal/engine/common/zigzag_pro2.py`
  - [ ] Step 1: Chèn khối logic kiểm tra nến hiện tại so với trạng thái `ob_waiting`.
  - [ ] Step 2: Xử lý các điều kiện Breakout High/Low theo Ma trận.
  - [ ] Step 3: Implement logic Double Pivot (Gán đồng thời cả `up` và `dn` buffer nếu cần).

- [ ] Task 3: Triển khai Quy tắc Thoát 5 nến (AC: 4)
  - Files
    - Modify: `services/aureus-signal/engine/common/zigzag_pro2.py`
  - [ ] Step 1: Tăng `ob_count` cho mỗi nến IB.
  - [ ] Step 2: Kiểm tra biên độ dao động của 5 nến gần nhất so với OB Range.
  - [ ] Step 3: Thực hiện vẽ cưỡng bức nếu thỏa mãn điều kiện.

- [ ] Task 4: Kiểm định và Xác nhận (AC: Tất cả)
  - Files
    - Test: `services/aureus-signal/tests/test_zigzag_ob_logic.py`
  - [ ] Step 1: Viết test case mô phỏng nến tin tức (OB cực to) và sideway.
  - [ ] Step 2: Chạy bộ test để xác nhận logic "Double Pivot" hoạt động đúng.
