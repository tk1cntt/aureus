...
- [ ] Task 1: Refactor `StructureSignal.calculate` to use persistent `t_map` (AC: 1, 2, 4)
  - Files
    - Modify: `services/aureus-signal/engine/signals/structure.py`
    - Test: `services/aureus-signal/tests/test_incremental_t_map.py` (New)
  - [ ] Step 1: Create a test that verifies `calculate` populates `state_obj.t_map`.
  - [ ] Step 2: Implement logic in `calculate`:
  ...
  - [ ] Step 3: Run test to verify `t_map` persistence across calls.

- [ ] Task 2: Implement Sliding Window Detection (AC: 3)
  - Files
    - Modify: `services/aureus-signal/engine/signals/structure.py`
  - [ ] Step 1: Add a check in `calculate`: if `len(state_obj.t_map) > len(df)` or the first timestamp doesn't match, clear and rebuild.
  - [ ] Step 2: Create a test case where `df` shifts (removes first row) and verify `t_map` correctly reflects new indices.

- [ ] Task 3: Performance Benchmarking (Internal) (AC: 4)
  - Files
    - Test: `services/aureus-signal/tests/test_incremental_t_map.py`
  - [ ] Step 1: Add a benchmark test comparing execution time of 1000 updates between the old "full rebuild" and new "incremental" logic.
...
### Fixes Applied
- [ ] Implemented integer key conversion in `SymbolState.from_dict`.
- [ ] Optimized `t_map` update loop using `astype(int)` on numpy array.
- [ ] Removed "magic number" buffer in stale check for exact match logic.
- [ ] Committed `tests/test_incremental_t_map.py` to git.
