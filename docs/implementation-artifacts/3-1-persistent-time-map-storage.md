...
- [x] Task 1: Add `t_map` to `SymbolState` initialization (AC: 1, 2)
  - Files
    - Modify: `services/aureus-signal/engine/state.py`
    - Test: `services/aureus-signal/tests/test_state_management.py` (New)
  - [x] Step 1: Create a test ensuring `SymbolState` has `t_map` after init and reset.
  - [x] Step 2: Run test to verify it fails (missing attribute).
    - Command: `python3 -m pytest services/aureus-signal/tests/test_state_management.py`
  - [x] Step 3: Add `self.t_map: Dict[int, int] = {}` to `SymbolState.reset()`.
  - [x] Step 4: Run test to verify it passes.

- [x] Task 2: Implement Serialization for `t_map` (AC: 4)
  - Files
    - Modify: `services/aureus-signal/engine/state.py`
  - [x] Step 1: Update `from_dict` to load `t_map` (using `.get('t_map', {})`).
  - [x] Step 2: Update `to_dict` to include `t_map` in the returned dictionary.
  - [x] Step 3: Add a test case in `test_state_management.py` for serialization parity.
