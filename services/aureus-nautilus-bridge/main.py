...
from reconciliation import normalize_execution_report
...
class NautilusAdapter:
    """Minimal adapter abstraction for Slice 1.

    In this slice, the adapter simulates an accepted execution report while preserving
    a clear contract for future real Nautilus integration.
    """
...
class AureusNautilusBridge:
    def __init__(self):
        ...
        self.processed_set_key = "aureus:bridge:processed_trace_ids"
    ...
    async def discover_streams(self):
        ...
        cursor = 0
        while True:
            cursor, keys = await self.redis.scan(cursor, match="aureus:stream:*:orders", count=100)
    ...
    async def _is_processed(self, trace_id: str) -> bool:
        if trace_id in self.processed_trace_ids:
            return True
        if await self.redis.sismember(self.processed_set_key, trace_id):
            self.processed_trace_ids.add(trace_id)
            return True
        return False

    async def _mark_processed(self, trace_id: str):
        self.processed_trace_ids.add(trace_id)
        await self.redis.sadd(self.processed_set_key, trace_id)
    ...
    async def handle_message(self, stream_name: str, msg_id: str, payload: Dict[str, Any]):
        ...
        try:
            ...
            if not trace_id or not symbol:
                raise ValueError("Missing trace_id/symbol in order payload")

            if await self._is_processed(trace_id):
                await self.redis.xack(stream_name, CONSUMER_GROUP, msg_id)
                return
            ...
            if event is not None:
                ...
                await self.redis.xadd(execution_stream, {
                    "type": "EXECUTION_EVENT",
                    "data": json.dumps(event),
                })
                await self._mark_processed(trace_id)
                logger.info("Published execution event for %s -> %s", trace_id, execution_stream)
