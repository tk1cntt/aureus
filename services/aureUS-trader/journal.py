    ...
    async def on_strategy_match(self, event: dict) -> bool:
        ...
        try:
            ...
            async with self.db.acquire() as conn:
                ...
                if result:
                    logger.warning("Reasoning entry deferred for trace_id=%s until parent trade and snapshot exist", trace_id)
        ...
        try:
            ...
            async with self.db.acquire() as conn:
                async with conn.transaction():
                    ...
                    # Extract scoring fields for evaluation record (Phase 55.1 restore)
                    ...
                    score_version_raw = event.get("score_version")
                    if isinstance(score_version_raw, str) and score_version_raw.strip():
                        score_version = score_version_raw.strip()
                    else:
                    ...
                    if trade_journal_id is not None:
                        ...
                        # Phase 55.1 restore: Insert evaluation record if scoring data present
                        if score_total is not None and trade_journal_id is not None:
                            try:
                                await conn.execute(
                                    """
                                    INSERT INTO aureus_trade_evaluations (
                                        trade_journal_id, trace_id, ticket, score_version,
                                        score_total, score_breakdown, weights_snapshot,
                                        missing_data_policy, strategy_name, symbol, timeframe,
                                        is_current
                                    ) VALUES (
                                        $1, $2, $3, $4, $5, $6::jsonb, $7::jsonb, $8, $9, $10, $11, TRUE
                                    )
                                ...
                                logger.info(
                                    "evaluation_record: trace_id=%s score_total=%s version=%s",
                                    trace_id,
                                    score_total,
                                    score_version,
                                )
                                ...
                                logger.warning(
                                    "evaluation_insert: trace_id=%s failed=%s",
                                    trace_id,
                                    eval_err,
                                )
                        try:
