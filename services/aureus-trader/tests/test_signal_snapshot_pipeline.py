...
async def test_signal_snapshot_persist_with_partial_signal_payload(journal_manager, mock_db_pool):
    ...
    assert snapshot_args[6] is None
    assert snapshot_args[7] is None
    assert '"nested": {"momentum": "up"}' in snapshot_args[4]


@pytest.mark.asyncio
async def test_order_opened_without_signal_payload_persists_from_journal_context(journal_manager, mock_db_pool):
    order_opened_event = {
        "type": "ORDER_OPENED",
        "trace_id": "tr-55-001",
        "ticket": 123456789,
    ...
    snapshot_args = snapshot_queries[0][2]
    assert snapshot_args[3] == "sig-v1.0.0"
    assert '"active_signals"' in snapshot_args[4]
    assert '"context_filters"' in snapshot_args[4]
