...
class TestSequenceMatching(unittest.TestCase):
    ...
    def test_choch_up_bos_up_sequence_matches(self):
        ...
        # Signal history: choch_up then bos_up
        state = MockState(signal_history=[
            {"tag": "choch_up", "t": 1000},
            {"tag": "bos_up", "t": 1010},
        ])
    ...
    def test_wrong_order_bos_before_choch(self):
        ...
        state = MockState(signal_history=[
            {"tag": "bos_up", "t": 1000},  # bos_up first
            {"tag": "choch_up", "t": 1010},  # choch_up second
        ])
...
class TestFullPipeline(unittest.TestCase):
    ...
    def test_full_pipeline_choch_then_bos_triggers_strategy(self):
        ...
        # Build signal_history from transient_signals events
        events = []
        for tag, sig in state.transient_signals.items():
            if isinstance(sig, dict) and sig.get("tag"):
                events.append({"tag": sig["tag"], "t": sig.get("t", 0)})

        state.signal_history = events

        df3 = pd.DataFrame({'t': [1202], 'o': [100], 'h': [105], 'l': [95], 'c': [102]})
