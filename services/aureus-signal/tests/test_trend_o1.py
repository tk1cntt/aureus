...
def test_trend_signal_sideways_both_colors():
    ...
    df = pd.DataFrame({'c': [100, 100, 100, 100, 100]})
    state = MockState()
    state.obs = [
...
def test_trend_signal_bearish_trend():
    ...
    df = pd.DataFrame({'c': [120, 115, 110, 105, 100]}) 
    state = MockState()
    state.obs = [
        {'ob_type': 'BEARISH', 'mitigated': False},
        {'ob_type': 'BEARISH', 'mitigated': False},
