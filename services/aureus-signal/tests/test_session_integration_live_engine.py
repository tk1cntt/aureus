...
class _FakeDBPool:
    def __init__(self):
        base = datetime.fromtimestamp(1704063600, tz=timezone.utc)
        self._warmup_rows = []
        for i in range(30):
            candle_time = base + timedelta(minutes=i)
            open_px = 2050.0 + (i * 0.25)
        ...
        if key == "aureus:state:XAUUSD":
            ...
            if isinstance(value, str):
                try:
                    ...
                    parsed = {}
                session_hlo = (parsed.get("tracking_vars") or {}).get("session_hlo") or {}
                if session_hlo.get("session") == "ASIA":
...
class TestSessionEnginePathIntegration(unittest.IsolatedAsyncioTestCase):
    async def test_run_signal_engine_persists_session_tracking_contract_in_runtime_loop(self):
